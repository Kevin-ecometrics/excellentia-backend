...
### Inventario / Almacén (módulo ya implementado — este encabezado quedó desactualizado, corregido 2026-09-08)

**Estado real verificado en código (2026-09-08):** el módulo existe completo en las tres partes — backend (`warehouseController.ts`: `ensureWarehouseTables`, `createReceipt`, `listLots`, `computeFifoAllocation`, `backfillLots`, `setLotCondition`, `updateLot`, `listMovements`, `recordMovement` con sync instantáneo a QBO), webapp (`app/warehouse/page.tsx` + `app/warehouse/inventory/page.tsx`) y Android (`WarehouseActivity`, `WarehouseRouteDetailActivity`, `ReceivingActivity`, `InventoryMovementsActivity`, `RouteReturnsActivity`, `ConsignmentActivity`). Construido en las Fases 111/112/114 — el ítem de abajo describía el estado de *antes* de esas fases, ya no aplica tal cual.

| Prioridad | Feature | Detalle |
|---|---|---|
| ✅ | ~~**Módulo de inventario**~~ | Hoy "stock" ya no es solo un contador — hay pantalla y endpoints dedicados en las 3 partes (ver arriba). Sub-puntos, todos cerrados: (1) `convertPreOrder` no descontaba stock — **✅ implementado**, ver fila abajo; (2) ~~recepción de mercadería~~ — **resuelto en la Fase 112**; (3) conteo físico / reconciliación con motivo registrado — ❌ descartado por ahora (2026-09-07), decisión del usuario: depende de si el cliente lo termina pidiendo. Verificado que `ADJUSTMENT` solo lo usa `updateLot()` (editar/devolver un lote puntual, sin motivo ni alcance por producto) — no cubre un conteo físico real, reconsiderar en el futuro si hace falta; (4) ~~historial de movimientos auditable~~ — **resuelto en la Fase 112**; (5) Lbs y peso real en stock — implementado y **revertido** el mismo día (bug real, ver fila abajo); (6) alertas de stock bajo con punto de reorden — ❌ descartado (2026-09-07), decisión del usuario: mantener el umbral fijo ≤5 ya aprobado en Webapp, sin punto de reorden configurable por producto; (7) daños de venta (`batch_damage`) — ❌ descartado (2026-09-07), decisión del usuario: el crédito en $ ya compensa al cliente, no hace falta descontar stock. Verificado que el gap es real (ni `batch_damage`/`computeDamageCredit` ni el crédito standalone de la Fase 76 tocan `products.stock`), queda así a propósito; (8) soporte offline — descartado a propósito, Almacén es online-only desde la Fase 112 |

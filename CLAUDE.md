...
**Implementado (2026-09-08):** `updateProduct` ahora expone si el push a QBO tuvo éxito. La respuesta del `PUT` suma `qb_synced` (`true`/`false`/`null` si no se intentó ningún push — sin `qb_item_id` o sin campos que sincronizan) y, si falló, `qb_sync_error` con el motivo (mismo criterio que `extractQboErrorMessage` de `orderController.ts` — combina `error.message` + `error.description`/`fault.errors[0].detail`, duplicado como función local en `productController.ts` en vez de exportado, mismo patrón ya usado para no acoplar controllers entre sí). Webapp (`ProductModal.tsx`) lee el campo: si `qb_synced === false`, no cierra el modal solo — muestra un banner ámbar con el motivo y cambia el footer a un solo botón "OK" que recién ahí cierra y refresca la lista, para que el admin no se entere recién la próxima vez que abra el producto.
